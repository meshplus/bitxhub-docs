...
=== "Fabric"
    ...
    ```
    cd ~/bitxhub-v1.6 && go get github.com/securekey/fabric-examples/fabric-cli/cmd/fabric-cli
    ```
...
=== "Ethereum"
    ...
    **二进制下载**
    ...
    除了源码编译外，我们也提供了直接下载Pier及其插件二进制的方式，下载地址链接如下：[Pier二进制包下载](https://github.com/meshplus/pier/releases/tag/v1.6.2) 和 [ethereum插件二进制包下载](https://github.com/meshplus/pier-client-ethereum/releases/tag/v1.6.2)链接中已经包含了所需的二进制程序和依赖库，您只需根据操作系统的实际情况进行选择和下载即可。
...
=== "Fabric"
    ...
    **二进制下载**
    ...
    除了源码编译外，我们也提供了直接下载Pier及其插件二进制的方式，下载地址链接如下：[Pier二进制包下载](https://github.com/meshplus/pier/releases/tag/v1.6.2) 和 [fabric插件二进制包下载](https://github.com/meshplus/pier-client-fabric/releases/tag/v1.6.2)链接中已经包含了所需的二进制程序和依赖库，您只需根据操作系统的实际情况进行选择和下载即可。
...
=== "Ethereum"
    ...
    └── validating.wasm
    ```
    **注意：如果在本地启动以太坊私链，需要用账户私钥替换account.key。**
    ```shell
    cp ~/.goduck/pier/ethereum/1.3.0/* $CONFIG_PATH/.pier/ether
    ```
    ...
    **说明**：
    ...
    - ethereum.toml是需要重点修改的，需要根据应用链实际情况填写 **ethereum网络地址**、**broker合约地址及业务合约abi**，**账户的key**，示例如下：
        ...
        - `addr`: 如果是本地启动的以太坊私链启动的应用链，地址应为`ws://localhost:8546`；
        - `name`：应用链名称，用户可自定义；
...
1. Pier命令行发起应用链注册
   ```
   # 以用户目录下的pier为例
   pier --repo $CONFIG_PATH/.pier appchain register --name=ethereum --type=ether --consensusType POS --validators=$CONFIG_PATH/.pier/ether/ether.validators --desc="ethereum appchain for test" --version=1.0.0
   # 发起注册后会打印出应用链id和提案id
...
2. 中继链节点依次投票
   ...
   bitxhub --repo $CONFIG_PATH/bitxhub//scripts/build/node2 client governance vote --id 0xcb33b10104cd217aAB4b302e9BbeEd1957EDaA31-0 --info approve --reason approve
   bitxhub --repo $CONFIG_PATH/bitxhub//scripts/build/node3 client governance vote --id 0xcb33b10104cd217aAB4b302e9BbeEd1957EDaA31-0 --info approve --reason approve
